...
| **현재 상태** | 민감도 분석을 거친 안정 후보를 정책탭에 반영하고 모델 고도화 단계 완료 |

---

## 2026-07-15 핵심 이정표

단순히 K-Means 결과를 지도에 표시하는 단계에서 멈추지 않고, **후보지가 조건 변화에도 반복적으로 나타나는지 검증한 뒤 안정 후보만 정책 검토 화면에 반영**했습니다.

* K값, 난수 시드, 거리 상한, 군위 처리 조건을 바꾼 민감도 분석 수행
* 군위·외곽 편향을 별도 권역과 보류 후보로 분리해 과도한 추천 방지
* EPSG:5179 투영 좌표와 기존 후보 좌표 비교로 거리 계산 안정성 확인
* 안정 후보 기반 자원 확충 시뮬레이션과 용어 해설을 정책탭에 반영
* 카카오맵, 행정동 폴리곤, 최적입지 마커와 상세 패널을 실제 브라우저에서 검증
* 프론트엔드 타입 검사·린트·단위 테스트 14건, 백엔드 단위·통합 테스트 20건 통과

현재 모델 결과는 확정 입지가 아니라 **현장 조사와 전문가 검토의 우선순위를 좁히는 정책 보조자료**로 동결했습니다.

---

## 프로젝트를 시작한 이유

1. **가족의 안전과 직결된 피부에 와닿는 문제의식 (옥포 거주 경험)**  
   현재 거주 중인 달성군 옥포읍은 응급의료 인프라가 취약한 지역입니다. 심혈관 질환을 앓고 계신 아버지께 응급 상황이 발생할 경우, 당장 의지할 수 있는 가장 가까운 응급실이 먼 거리의 나사렛병원뿐이라는 현실적인 불안감과 불편함이 이 데이터 분석 프로젝트를 시작하게 된 가장 큰 원동력이 되었습니다.
2. **병원의 개수와 실제 접근성의 괴리**  
...
| **후보 민감도 분석** | K값·난수 시드·거리 상한·군위 처리 조건을 변경하며 반복 출현하는 안정 후보와 별도 검토 권역 분리 | 구현 완료 |
| **자원 확충 시뮬레이션** | 도출된 거점 반경 5km 내 기존 병원 인프라를 분석해 필요 자원(의사, MRI/CT) 및 우선순위(HIGH/MED/LOW) 자동 계산 | 구현 완료 |
| **정책 검토 시각화 대시보드** | 안정 후보, 설명용 점수, 분석 수요, 접근성 개선량과 용어 해설을 Kakao Maps 기반 정책탭에서 제공 | 구현 완료 |
...
* **후보 안정성 검증**: K값 2~5, 난수 시드 5종, 거리 상한 4종, 군위 포함·분리·제외 조건을 조합해 후보 반복 출현률을 비교.
* **좌표계 비교 검증**: EPSG:5179 기반으로 재계산한 후보와 기존 공개 후보의 위치 차이를 비교해 좌표 변환에 따른 급격한 결과 왜곡 여부 확인.
* **서비스 검증**: 프론트엔드 타입 검사·린트·단위 테스트 14건과 백엔드 단위·통합 테스트 20건을 통과하고, 정책탭 지도 상호작용을 실제 브라우저에서 확인.

---

## 한계

* **도메인 전문성의 부재 (PoC 목적)**: 작성자는 의료 보건 및 행정 전문가가 아닌, 이제 막 개발을 배우기 시작한 **부트캠프 1개월 차 교육생**입니다. 따라서 본 프로젝트에서 제시된 3중 복합 가중치 설계나 자원 시뮬레이션 로직은 실제 현장의 복잡한 의료법적, 행정적 제약을 모두 반영하지 못한 기술적 가설(Proof of Concept)에 불과합니다. 완벽한 정책 제안보다는 **'짧은 기간 동안 배운 데이터 분석과 대시보드 시스템 구축 기술을 바탕으로 문제를 스스로 정의하고 구현해 보는 도전 과정'**을 보여주는 데 주된 목적이 있습니다.
...
* **단기**: 모델을 동결하고 발표 문구, 데이터 출처, 재현 절차와 정책적 한계를 정리해 시연 안정성에 집중.
* **중기**: 비용이 발생하지 않는 OSRM 등 공개 도로망을 검토해 직선거리와 실제 이동 경로의 차이를 별도 실험으로 비교.
* **장기**: 의료·행정 전문가 검토와 신뢰할 수 있는 최신 데이터가 확보된 경우에만 가중치 재설계와 양방향 정책 시뮬레이터를 후속 연구로 진행.
