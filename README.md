...
*   대안 산식 순위 검증에서 인구 로그 곱셈식은 기준선과 Spearman 0.518·상위 10개 겹침 2/10, 로그 구성요소 동일가중 정규화식은 0.935·7/10으로 확인됐습니다. VDI는 확정 서열이 아니라 가중 ETA·15분 커버율과 함께 보는 내부 우선순위 지표입니다.
...
*   p-median 3개 후보를 기존 기관과 함께 적용한 모델에서 취약인구 가중 평균 ETA는 소아 15.932→12.969분, 어르신 11.890→11.155분으로 감소했습니다.
*   같은 조합에서 전체 체계의 15분 커버율은 소아 42.94→66.25%, 어르신 79.37→84.24%로 증가했습니다. 30분 커버율은 외곽권 누락 감시용 보조 지표로 사용합니다.
...
ENABLE_PUBLIC_DATA_SCHEDULER=true
USE_MOCK_API=false
BED_CACHE_POLL_INTERVAL_SEC=600
...
백엔드가 시작될 때 `ENABLE_PUBLIC_DATA_SCHEDULER=true`이면 공공데이터 예약 작업을 등록합니다. 현재 일정은 응급기관 매일 03:00, 달빛어린이병원 매일 03:15, 인구 매월 2일 04:00, 행정동 경계 매월 1일 05:00이며 모두 `APP_TIMEZONE`을 따릅니다. 실행 1회당 외부 요청은 병원 100회·인구 60회·SGIS 60회로 제한되고, 각 작업은 최대 10분 후 취소됩니다. `max_instances=1`과 DB 작업 잠금도 중첩 실행을 차단합니다. 실시간 병상 폴러는 기본 600초, 허용 범위 300~3,600초이며 갱신 1회가 120초를 넘으면 취소됩니다. 시작 후 `GET /health`에서 스케줄러 3개 상태 필드와 `bedCachePollerConfigured`, `bedCachePollerEnabled`, `bedCachePollIntervalSec`, `databaseStorageMode`를 확인합니다. API 키와 관리자 토큰은 저장소에 커밋하지 않고 로컬 `.env` 또는 배포 서비스의 비밀 환경변수로만 설정합니다.

2026-07-26 운영계정 확인 기준 중앙의료원 API는 엔드포인트별 일 1,000회, 나머지 연동 API는 일 10,000회다. 600초 병상 폴링은 서버가 계속 실행돼도 중앙의료원 엔드포인트당 정상 경로 약 144회/일(14.4%)이며, 네트워크 오류로 최대 3회 재시도를 매 주기 소진하는 엔드포인트도 이론상 약 432회/일(43.2%)이다. 배치 수집은 이와 별도로 일·월 1회와 실행별 요청 상한을 적용한다.

Render 무료 Web Service는 유휴 시 중지되고 로컬 SQLite가 영속 저장되지 않으므로, 예약 작업은 포트폴리오 시연을 위한 최선 노력 방식으로만 취급합니다. 무료 배포에서는 정적 검증본을 기준자료로 유지하고, 최신 운영정보가 필요하면 발표 전에 수동 갱신 성공 여부를 확인합니다. 24시간 자동 운영과 동적 데이터 보존이 필요하면 영구 저장소를 별도로 구성해야 합니다.
...
*   **인구 공표주기:** 주민등록 인구는 매월 말일 기준 자료를 익월 1일 12시 이후 공표합니다. 2026-07-26 기준 2026.06은 최신 공표 완료월이며, 2026.07 자료는 8월 공표·검증 후 반영합니다.
*   **VDI 구성요소 민감도:** 현재 150개 행정동 분포에서는 취약인구의 영향이 ETA보다 강합니다. 대안 산식에서 최대 순위 이동이 인구 로그 134위·동일가중 정규화 70위로 확인돼, 외곽 지역의 중요도를 단일 VDI 순위만으로 판단하지 않습니다.
