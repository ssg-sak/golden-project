...
이 프로젝트는 프론트엔드, 백엔드, 데이터 분석 영역으로 구성되어 있습니다.

### 💻 프론트엔드
- **핵심 라이브러리 및 타입 시스템:** 리액트 19 (v19.2.0), 타입스크립트 (v5.9.x), HTML5 / CSS3
- **빌드 및 개발 도구:** 바이트 8 (v8.1.3), 타입스크립트-이에스린트, 이에스린트
- **상태 관리:** 주스탠드 (v5.0.14) — 클라이언트 상태 관리 및 데이터 동기화
- **라우팅:** 리액트 라우터 돔 (v7.18.1)
- **스타일링 및 애니메이션:** 테일윈드 CSS v4 (v4.3.2) — 화면 스타일링, 프레이머 모션 (v12.42.2) — 컴포넌트 전환 효과
- **지도 엔진 및 공간 정보:** react-kakao-maps-sdk (v1.2.1) — 카카오맵 API 연동 및 마커 렌더링
- **테스트 검증:** 바이테스트 (v3.2.4) — 단위 테스트, 플레이라이트 (v1.52.0) — 종단간(E2E) 시나리오 테스트

### ⚙️ 백엔드
- **웹 프레임워크:** 패스트API (v0.110.0 이상) — 비동기 API 서버 구현
- **서버 구동기:** 유비콘 (v0.27.0 이상)
- **데이터베이스 및 객체 관계 매핑:** SQLite (내장 캐시용 데이터베이스), SQL알케미 (v2.0.0 이상)
- **작업 스케줄러:** AP스케줄러 (v3.10.0) — 실시간 병상 정보 2분 주기 백그라운드 수집 캐싱
- **API 데이터 수집 및 파싱:** xmltodict (공공 API XML 파싱), 리퀘스트 및 HTTPX (HTTP 요청 처리)
- **테스트 검증:** 파이테스트 (v8.0.0 이상)

### 📊 데이터 분석 및 공간 분석
- **데이터 처리 및 조작:** 판다스 (v2.0.0 이상) — 병원 및 자원 데이터 전처리
- **지리 공간 분석:** 지오판다스 (v0.14.0 이상) — 대구 격자 데이터 지리 공간 연산
- **기계학습 및 분석 알고리즘:** 사이킷런 — K-평균(K-Means) 군집화를 통한 의료 사각지대 입지 분석 및 지수 도출
