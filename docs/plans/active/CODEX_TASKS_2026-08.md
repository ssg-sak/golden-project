...
자가검증 관문 (⭐⭐면접 최우선 — 이 TASK의 진짜 목적)

이 작업은 코드 개선이 목적이 아니라 "pandas를 쓸 줄 안다"는 증거를 만드는 것이 목적입니다. Codex가 짜준 코드를 이해하지 못하면 아무 의미가 없습니다.
...
면접관이 저장소를 열었을 때 모순되어 보이는 파일들이 남아 있습니다. 특히 08_compute_vulnerability_geojson.py는 로그 변환이 없는 구식 VDI 산식을 그대로 갖고 있어, 프로젝트의 핵심 강점(로그 변환 도입)과 정면으로 충돌합니다.
...
2. Codex에게 절대 맡기면 안 되는 것

이게 이 문서에서 가장 중요한 부분입니다.

항목	왜 직접 해야 하는가
SQL 학습 및 indicators.py·dashboard.py의 SQL 전환	이 작업의 목적은 코드 개선이 아니라 SQL 실력을 만드는 것입니다. Codex가 써주면 결과물은 좋아지지만 코딩테스트에서는 그대로 떨어집니다. 쿼리는 직접 쓰세요
pandas 학습 (groupby().agg(), pivot_table 등)	위와 동일. TASK 4는 "증거 만들기"용이고, 실력은 별도 훈련이 필요합니다
면접 답변 대본	본인 언어로 써야 면접장에서 나옵니다
methodology.md의 p-median/MCLP 설명 추가	코드를 읽고 본인이 이해한 내용을 본인 문장으로 써야 합니다. Codex가 쓰면 면접에서 그대로 무너집니다
포트폴리오 문구	수정 불가 상태이며, 어차피 대응은 코드 쪽에서 합니다
3. 진행 체크리스트
순서	TASK	브랜치	상태	자가검증
...
TASK 2·3·4를 마치면 포트폴리오를 한 글자도 고치지 않고 면접 위험 요소가 5개에서 2개로 줄어듭니다.
