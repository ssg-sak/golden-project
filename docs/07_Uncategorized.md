...
> **⚠️ 현재 구현과 다른 과거 기록 (2026-07-18):** 이 문서는 분류되지 않은 개발 메모와 작업 결과를 보관한 아카이브입니다. 제거된 코드·데이터 경로 및 과거의 `진료 가능`, `수용 불가`, `완벽한 정책 해답` 같은 표현이 남아 있을 수 있으며 현행 서비스 주장으로 사용하지 않습니다. 현재 기준은 `README.md`, `docs/PROJECT_STRUCTURE.md`, `docs/methodology.md`입니다.
