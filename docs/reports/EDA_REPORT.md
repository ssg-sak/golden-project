...
이 문서는 `2026-07-r1` / 인구 기준월 `2026.07` 정책 릴리스를 탐색적으로 점검합니다. EDA는 현재 데이터의 분포와 변수 관계를 설명하며 의료적 위험 임계값이나 시설 신설 효과를 확정하지 않습니다.
...
**해석(Insights)**:
- 도로 ETA 기반 VDI는 749.83~54,858.43, 평균 10,386.40, 중앙값 9,478.42입니다.
- 현재 분석본은 VDI 상위 25%를 우선 확인 대상으로 구분하며 상대 경계값은 13,429.72, 해당 행정동은 38개입니다.
...
**해석(Insights)**:
- 현재 VDI와 취약인구의 피어슨 상관계수는 0.933, 도로 ETA와의 상관계수는 0.041입니다.
...
|---|---:|---:|---:|---:|
| `ln(1+ETA) × ln(1+취약인구)` | 0.535 | 3/10 | 19.5위 | 134위 |
| 로그 구성요소 Min-Max 정규화 후 동일가중 합 | 0.895 | 6/10 | 9.0위 | 85위 |

- **정책적 시사점**: 인구 로그 곱셈 대안은 상위 10개 중 3개를 유지하고 순위상관이 0.535이어서 현재 우선순위가 산식 선택에 민감합니다. 동일가중 정규화 대안은 상위 10개 중 6개를 유지하며 최대 85위 이동이 남습니다. 따라서 현재 VDI는 확정 서열이 아니라 한 가지 내부 우선순위 기준으로만 사용하고, 가중 ETA·15분 커버율과 함께 판단해야 합니다.
...
**해석(Insights)**:
- 현재 상위 3개는 달성군 다사읍, 달성군 화원읍, 달서구 진천동입니다.
...
**해석(Insights)**:
- 소아 후보 1, 3, 4 적용 시 취약인구 가중 평균 ETA는 16.237분에서 13.286분으로 줄고, 전체 체계의 15분 커버율은 41.44%에서 64.01%로 증가합니다.
- 어르신 후보 1, 2, 3 적용 시 취약인구 가중 평균 ETA는 12.034분에서 11.269분으로 줄고, 전체 체계의 15분 커버율은 79.19%에서 84.00%로 증가합니다.
- 전체 체계의 30분 커버율은 소아 97.93%→98.76%, 어르신 93.81%→94.93%입니다. 기준선부터 높아 주 KPI가 아니라 외곽권 누락 감시용 보조 지표로 둡니다.
