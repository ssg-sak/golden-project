...
- **정책적 시사점**: 인구 로그 곱셈 대안은 상위 10개 중 3개를 유지하고 순위상관이 0.535여서 현재 우선순위가 산식 선택에 민감합니다. 동일가중 정규화 대안은 상위 10개 중 6개를 유지하며 최대 85위 이동이 남습니다. 따라서 현재 VDI는 확정 서열이 아니라 한 가지 내부 우선순위 기준으로만 사용하고, 가중 ETA·15분 커버율과 함께 판단해야 합니다.
