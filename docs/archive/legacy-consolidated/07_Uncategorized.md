...
> **⚠️ 현재 구현과 다른 과거 기록 (2026-07-18):** 이 문서는 분류되지 않은 개발 메모와 작업 결과를 보관한 아카이브입니다. 제거된 코드·데이터 경로 및 과거의 `진료 가능`, `수용 불가`, `완벽한 정책 해답` 같은 표현이 남아 있을 수 있으며 현행 서비스 주장으로 사용하지 않습니다. 현재 기준은 `README.md`, `docs/core/PROJECT_STRUCTURE.md`, `docs/core/methodology.md`입니다.
...
- [x] **공공데이터 API 과다 호출(429 오류) 최적화**: 1주기당 9개 구/군을 개별 순회하여 18회씩 발생하던 불필요한 폴링을 대구시 통합 1회 호출(`numOfRows=200`)로 줄여 트래픽 부하를 **88.8% (1/9 수준) 절감**
...
# 사건 보고서 및 AI 에이전트 회고 (2026.07.12)
...
2. **부적절한 터미널 도구 사용으로 인한 서버 중단:** 텍스트 교체 과정에서 파워쉘 기반 문자열 치환(`python -c`)의 이스케이프 오류가 발생했고, 프론트엔드 빌드 실패와 서버 중단으로 이어졌습니다.
...
## 2. AI 에이전트 (Antigravity) 회고 (Retrospective)

오늘 작업 과정에서 검증과 전체 흐름 확인이 부족했던 점을 다음과 같이 정리합니다.

첫째, **백엔드 코드만 수정하고 프론트엔드 UI 반영을 확인하지 않았습니다.** 데이터 파이프라인의 끝은 사용자가 보는 화면이므로, 서버 수정 이후 실제 화면까지 확인하는 전체 구간 검증이 필요합니다.

둘째, 화면 교체 과정에서 안전한 전용 수정 도구 대신 문자열 치환 스크립트를 사용해 개발 서버 장애를 유발했습니다. 이후에는 변경 범위가 명확한 전용 도구를 우선 사용해야 합니다.
...
넷째, 공식 API 명세를 확인하지 않고 `hvoc`, `hvcc` 필드의 의미를 추정해 잘못된 스키마를 제공했습니다. 공공데이터 필드는 반드시 공식 명세와 실제 응답을 교차 확인해야 합니다.
...
이번 피드백을 통해 전체 구간 검증과 공식 명세 확인의 중요성을 재확인했습니다. 앞으로는 화면 반영과 실행 검증까지 완료한 뒤 결과를 보고하겠습니다.
...
이 계획은 산출된 k=2 최적 거점 데이터를 바탕으로, 대시보드 우측 보드(`DetailPanel`)에 **정확한 주소와 커버리지 정보**를 시각화하기 위한 설계입니다. 프로젝트에서 정한 "상태와 뷰의 분리", "목적에 맞는 컴포넌트", "중복 로직 최소화" 원칙을 준수합니다.
...
4. 디자인이 합의된 품질 기준과 일관된 UI(Aesthetics)를 유지하는지 점검합니다.
...
### [비용 급증 방지 및 최적화 전략 (Cost-Saving Strategy)]
...
- **[Task 3] 기술 이력서 및 블로그화:** 캐싱 방어, K-Means 파인튜닝, 렌더링의 세 가지 강점을 C4 다이어그램과 함께 포트폴리오용 문서로 기록.
...
- **캐싱(Cache) 적용:** 24개 병원에 대한 N+1 쿼리 급증을 막기 위해 서버단 `asyncio.gather` 및 `cachetools` 적용.
...
| **식당 비유** | 한 명의 주방장이 요리 1개를 끝낼 때까지 다음 주문을 받지 못하고 줄을 서서 기다림. | 요리가 진행되는 동안(외부 API 응답 대기) 다른 주문을 먼저 처리하는 웨이터(Uvicorn) 역할. |
...
## 1. 개요
사용자 요구에 맞추어 시스템 내부의 세부 구조가 아닌 **[프론트엔드] - [백엔드] - [데이터베이스]** 전체를 아우르는 매크로(Macro) 수준의 아키텍처 다이어그램을 다시 작성했습니다. 단일 코드 수정 없이 전체 시스템의 상호작용을 보여줍니다.
...
사용자 피드백대로, 이는 컴포넌트 분리와 신규 기능 개발 과정에서 발생한 **구조적 접근 오류**입니다.
...
이 문제를 조기에 확인해 연쇄적인 아키텍처 오류를 막을 수 있었습니다. 회고 내용을 바탕으로 구조 검증과 원칙 준수를 강화합니다.
...
해당 이슈는 FSD 원칙에 따라 `shared/api` 폴더를 `data/api`로 이동한 직후, 개발 환경에서 **"해당 형식 선언을 찾을 수 없습니다(Cannot find module / Cannot find name)"** 오류가 다수 발생하며 화면이 표시되지 않은 현상에 대한 사후 분석입니다.
...
전체 시스템 취약점을 추가 점검(`grep_search` 등)한 결과, 즉시 오류를 표시하지 않지만 데이터 오염을 유발할 수 있는 잠재 문제를 찾아 교정했습니다.
