...
function ResultRow({ label, result }: { label: string; result: ObjectiveResult }) {
  return (
    <div className="grid grid-cols-[5.5rem_1fr] items-center gap-2 border-t border-slate-100 py-2 first:border-t-0">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-right text-[10px] font-semibold text-slate-500">
          {label === 'p-median'
            ? `${result.weighted_average_eta_minutes.toFixed(1)}분`
            : label === 'MCLP 15분'
              ? `${(result.covered_15min_ratio * 100).toFixed(1)}%`
              : `${(result.covered_30min_ratio * 100).toFixed(1)}%`}
        </span>
      </div>
    </div>
  );
    ...
    <section className="pointer-events-auto absolute left-3 top-3 z-30 w-[min(22rem,calc(100%-1.5rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-[0_18px_48px_rgba(15,23,42,0.18)] backdrop-blur-md sm:left-4 sm:top-4">
      <div className="bg-[linear-gradient(125deg,#0f172a,#173b4f)] px-4 py-3 text-white">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200">도로망 입지 최적화</p>
            <p className="mt-1 text-sm font-extrabold">
              {currentMode === 'pediatric' ? '소아 응급 접근성' : '고령층 응급 접근성'}
            </p>
          </div>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[11px] font-bold text-slate-600">선정할 시설 수</p>
        ...
        <div className="mt-2">
          <ResultRow label="p-median" result={selected.p_median_optimum} />
          <ResultRow label="MCLP 15분" result={selected.mclp_15min_optimum} />
          <ResultRow label="MCLP 30분" result={selected.mclp_30min_optimum} />
        </div>

        <p className="mt-2 rounded-lg bg-amber-50 px-2.5 py-2 text-[10px] leading-relaxed text-amber-900 ring-1 ring-amber-100">
          K-Means 안정 후보군의 모든 조합을 비교한 결과입니다. 후보 번호와 지도 마커를 대조하면 단일 후보 지표를 함께 확인할 수 있습니다.
        </p>
      </div>
    </section>
  );
