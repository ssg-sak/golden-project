...
### 🗺️ 카카오맵 연동 (react-kakao-maps-sdk) 및 길안내
...
- **카카오내비 외부 앱 연동 (Deep Link)**: 인앱(In-app) 라우팅 연산의 부하를 줄이고 정확한 실제 도로망 기반의 ETA를 확보하기 위해, `kakaoDirectionsUrl` 모듈을 통해 모바일/웹 환경에서 카카오내비 앱으로 즉시 연결되는 URL Scheme을 제공합니다.
...
본 시스템은 프론트엔드가 공공데이터(HIRA/E-Gen) API를 직접 찌르는 허술한 방식을 배제하고, 무중단 실시간 연동 체계를 갖췄습니다.
- **백그라운드 실시간 폴링 (Polling)**: 백엔드의 `bed_poller.py`와 `hira_client.py`가 주도적으로 국립중앙의료원과 심평원 API를 1~2분 주기로 백그라운드에서 폴링하여 최신 상태를 유지합니다.
- **서킷 브레이커 (Circuit Breaker) 패턴**: 공공 API 지연 발생 시 클라이언트 요청이 병목에 걸리지 않도록 3초 서킷 브레이커를 적용하여, 시간 초과 시 캐시된 SQLite 데이터나 안전한 Fallback 응답을 즉시 반환합니다.
- 프론트엔드는 오직 백엔드가 가공해 둔 안정된 JSON 페이로드만을 전달받아 렌더링함으로써 속도와 보안(API Key 숨김)을 동시에 확보했습니다.
