...
export function HeatmapToggle() {
  ...
  return (
    ...
    >
      <span>취약지구 히트맵</span>
      <span
        className={`inline-flex h-5 w-9 shrink-0 items-center overflow-hidden rounded-sm p-0.5 transition-colors ${
          showHeatmap ? 'justify-end bg-white/30' : 'justify-start bg-slate-200'
        }`}
        aria-hidden
      >
        <span className="h-3.5 w-3.5 shrink-0 rounded-sm bg-white shadow-sm" />
      </span>
    </button>
  );
}
