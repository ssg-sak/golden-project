  ...
  if (!hasData) {
    return (
      <section className="shrink-0 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
        <h3 className="text-sm font-bold text-slate-700">병원 진료자원 정보</h3>
        <p className="mt-2 text-xs leading-relaxed text-slate-500">
          현재 이 병원의 의료진, 장비 보유, 운영시간 정보가 없습니다. 실제 이용 전에는 병원에 직접 확인해 주세요.
        </p>
    ...
    <section className="shrink-0 border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-extrabold text-slate-900">병원 진료자원 정보</h3>
        </div>
        {hospital.hira_reference_date ? (
          <span className="text-[9px] font-semibold text-slate-500">기준일 {hospital.hira_reference_date}</span>
        ) : hospital.hira_source === 'api' ? (
          <span className="text-[9px] font-semibold text-slate-400">공공데이터 조회</span>
        ) : null}
      </div>
      ...
      <div className="space-y-3">
        {hospital.hira_equipment_status === 'failed' ? (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] leading-relaxed text-amber-900">
            <strong className="font-extrabold">장비 상세 조회 실패</strong>
            <p className="mt-1">
              {hospital.hira_equipment_message ??
                '장비 상세 정보가 일시적으로 응답하지 않아 등록 장비 보유 현황을 확인하지 못했습니다.'}
            </p>
        ...
        {hospital.hira_equipment_status === 'snapshot' ? (
          <div className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-[11px] leading-relaxed text-sky-900">
            <strong className="font-extrabold">저장된 기준일 자료 표시 중</strong>
            <p className="mt-1">
              {hospital.hira_equipment_message ??
                '실시간 장비 상세 조회 대신 기준일이 확인된 보유 장비 정보를 표시합니다.'}
            </p>
          </div>
        ) : null}
        ...
        {emergencyEquipment.length > 0 ? (
          <div className="border-l-4 border-teal-700 bg-slate-50 p-3">
            <p className="text-xs font-extrabold text-slate-900">응급진료 핵심장비 현재 가능 여부</p>
            <p className="mb-2 mt-1 text-[10px] leading-relaxed text-slate-500">
              응급진료에 자주 확인되는 영상검사, 예관검사, 호흡 보조 장비입니다. 실제 사용 가능 여부는 병원 확인이 필요합니다.
            </p>
            <ul className="grid grid-cols-2 gap-1.5">
              {emergencyEquipment.map(([name, available]) => (
                <li
                  key={name}
                  className={`rounded-lg px-2 py-1.5 text-[10px] font-bold ring-1 ${
                    available ? 'bg-emerald-50 text-emerald-800 ring-emerald-200' : 'bg-slate-950 text-white ring-slate-950'
                  }`}
                >
                  {name} · {available ? '가능' : '현재 불가'}
                </li>
              ))}
            </ul>
        ...
        {equipment.length > 0 && emergencyEquipment.length === 0 ? (
          <div>
            <p className="text-[10px] font-bold text-slate-600">등록 장비 보유 현황</p>
            <p className="mb-2 mt-1 text-[10px] leading-relaxed text-slate-400">
              정기 등록자료의 보유 여부이며, 실제 현재 사용 가능 여부와 다를 수 있습니다.
            </p>
          ...
          <div className="overflow-hidden rounded-xl bg-slate-50 ring-1 ring-slate-200">
            ...
            >
              <span className="text-[10px] font-bold text-slate-500">
                공공데이터 안내사항 {hospital.hira_notices.length}건
              </span>
        ...
        <p className="text-[10px] leading-relaxed text-amber-700">
          현재 화면은 참고용입니다. 실제 사용 가능 여부는 최신 상황을 병원에 직접 확인해 주세요.
        </p>
      </div>
    </section>
  );
